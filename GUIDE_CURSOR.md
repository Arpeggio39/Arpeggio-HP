...
`v26.x.x` と pnpm 11系のバージョンが表示されれば準備完了です。バージョンが表示されない場合は連絡してください。
...
次に、出現した入力フィールドに、着手する内容のタイトル？を簡単に設定する
...
そしてEnterを押して、さっき押した左下の文字が **「add_album_info」**に切り替わっていたらOK！

次に、左下のブランチ名のすぐ右にある、ブランチ作成ボタン（雲に上向き矢印が刺さってるボタン）を押します。

これで開発の準備は完了！！！！
**あとは、自分が担当している部分をカタカタ開発しよう！！！！！**

## とにかく開発に着手するときは、ブランチが着手する内容のタイトル？になっているか確認
...
- GitHubアカウント作成
- Git、Node.js、pnpmのインストール
- Cursorのインストール
- ペジオHPのコードをGitHubからClone
- 必要なパッケージをインストール（pnpm install --frozen-lockfile）

## 開発フェーズで毎回行う作業

- 左下にある同期ボタンを押して、最新のデータを取得
- #(番号)という名前のブランチを作成
- コード編集・開発
- コミット & プッシュ（適宜）
- 担当機能完成後にPull Requestの作成

PRが承認されたら、#(番号)のブランチは自動で削除します！
また新しく担当する場合、**開発フェーズで毎回行う作業**(左下の同期ボタンを押してブランチ作成するところ)から始めてください！
